...
init_name_list = ['mammal', 'bird', 'device', 'container']
random_order_list = []
while len(init_name_list) > 0:
    rand_ind = random.randint(0, len(init_name_list)-1)
    chosen_ele = init_name_list[rand_ind]
    tmp = data_name_hier_dict_100_trial3[chosen_ele]
    if not len(tmp) == 0:
        random_order_list.append(chosen_ele)
    data_name_hier_dict_100_trial3.pop(chosen_ele)
    data_name_hier_dict_100_trial3 = dict(data_name_hier_dict_100_trial3, **tmp)
    init_name_list = list(data_name_hier_dict_100_trial3.keys())

# print(random_order_list)
# seed 0
# ['container', 'self-propelled_vehicle', 'mammal', 'bag', 'canine', 'rodent',
# 'other_wheeled_vehicle', 'ungulate', 'box', 'feline', 'primate', 'vessel',
# 'device', 'bird', 'wading_bird', 'other_oscine', 'game_bird', 'restraint',
# 'musical_instrument', 'finch', 'other_aquatic_bird', 'machine', 'mechanism', 'instrument']

# seed 1
# ['bird', 'device', 'wading_bird', 'finch', 'restraint', 'other_oscine', 'mammal', 'canine',
# 'container', 'box', 'other_wheeled_vehicle', 'vessel', 'other_aquatic_bird', 'instrument',
# 'mechanism', 'game_bird', 'feline', 'machine', 'bag', 'self-propelled_vehicle', 'ungulate', 'musical_instrument',
# 'primate', 'rodent']


class_dict = {'root':['mammal', 'bird', 'device', 'container'],
              'mammal':['ungulate', 'rodent', 'primate', 'feline', 'canine'],
              'bird': ['game_bird', 'finch', 'wading_bird', 'other_oscine', 'other_aquatic_bird'],
              'device': ['instrument', 'restraint', 'mechanism', 'musical_instrument', 'machine'],
              'container': ['vessel', 'box', 'bag', 'self-propelled_vehicle', 'other_wheeled_vehicle'],
...
res_list = [class_dict['root']]
coarse_ind_list = [0]
for ind in range(len(random_order_list)):
    curr_ele = random_order_list[ind]
    res_list.append(class_dict[curr_ele])
    if curr_ele in ['mammal', 'bird','device', 'container']:

        coarse_ind_list.append(ind+1)
print(res_list)

print(coarse_ind_list)
...
# seed  2
# ['mammal', 'device', 'container', 'vessel', 'canine', 'machine', 'musical_instrument', 'restraint', 'rodent', 'self-propelled_vehicle', 'primate', 'ungulate', 'feline', 'other_wheeled_vehicle', 'bag', 'instrument', 'box', 'mechanism', 'bird', 'finch', 'game_bird', 'wading_bird', 'other_aquatic_bird', 'other_oscine']
