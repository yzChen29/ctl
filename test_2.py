...
if __name__ == '__main__':
    ...
    df = pd.read_csv('~/ctl/Imagenet100_joint_task_0.csv')

    class_index_list = [int(i) for i in list(df['class_index'])[1:]]
    class_acc_list = list(df['avg_acc'])[1:]
    class_name_list = [data_label_index_dict_inv[i] for i in class_index_list]
    pair_name_acc = list(zip(class_name_list, class_acc_list))
    pair_name_acc.sort(key=lambda x: x[1])
    ...
    for i in pair_name_acc:
        parent_node_i = taxonomy_tree.nodes[i[0]].parent
        grad_node_i = taxonomy_tree.nodes[parent_node_i].parent
        if not parent_node_i in layer_2_acc_dict:
            layer_2_acc_dict[parent_node_i] = [i[1], 1]
        else:
            layer_2_acc_dict[parent_node_i][0] += i[1]
            layer_2_acc_dict[parent_node_i][1] += 1

        if not grad_node_i in layer_1_acc_dict:
            layer_1_acc_dict[grad_node_i] = [i[1], 1]
        else:
            layer_1_acc_dict[grad_node_i][0] += i[1]
            layer_1_acc_dict[grad_node_i][1] += 1
        
    for i in layer_1_acc_dict:
        layer_1_acc_dict[i][0] /= layer_1_acc_dict[i][1]
    for i in layer_2_acc_dict:
        layer_2_acc_dict[i][0] /= layer_2_acc_dict[i][1]

    for i in layer_2_acc_dict:
        grad_node_i = taxonomy_tree.nodes[i].parent
        if not grad_node_i in layer1_rank_dict:
            layer1_rank_dict[grad_node_i] = [(i, layer_2_acc_dict[i][0])]
        else:
            layer1_rank_dict[grad_node_i].append((i, layer_2_acc_dict[i][0]))

    for i in layer1_rank_dict:
        layer1_rank_dict[i].sort(key=lambda x: x[1])
        
    print(layer_1_acc_dict)
    print(layer_2_acc_dict)
    print(layer1_rank_dict)
